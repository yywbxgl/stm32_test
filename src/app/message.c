...
//生成指令1，定时保活信息
void create_keep_alive_message(char *outbuf, u16 len)
{
    char* out;
...
//生成指令2，开始刷卡消费信令
void create_start_consume_message(char *outbuf, u16 len)
{
...
void create_consume_message(char *outbuf, u16 len, u8 ic_flag, u8 finish_flag)
{
    char* out;
    json_t* message = json_object();
    json_t* param = json_object();
...
//生成指令5,提交离线消费记录
void create_outline_consume_message(char *outbuf, u16 len)
{
...
void create_start_app_consume_response(char *outbuf, u16 len, s8 ok_flag, char* orderNo)
{
    char* out;
    json_t* message = json_object();
    json_t* param = json_object();
...
u8 parse_keep_alive_response(u8 *outbuf, u16 len)
{
    ...
    json_t* param = json_object_get(message, "data");
